...
const MOBILE_NO_HEADER_PAGES = ['/search', '/orders', '/returns', '/reviews', '/favorites', '/addresses', '/payments'];
const MOBILE_NO_FOOTER_PAGES = ['/orders', '/returns', '/reviews', '/favorites', '/addresses', '/payments', '/search'];
...
export default function ShopLayout({ children }: { children: React.ReactNode }) {
  ...
  const mobileHideFooter = MOBILE_NO_FOOTER_PAGES.some((p) => pathname.startsWith(p));
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {showFooter && <Footer className={mobileHideFooter ? 'hidden sm:block' : ''} />}
    </div>
  );
}
